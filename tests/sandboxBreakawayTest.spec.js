...
const { kargoLink, breakawayAdLink, breakawayDemoAdLink } = require('../Ads Info/Ads Links');

test.describe('Sandbox Breakaway Ad Page', () => {
  test('Check and Click on Kargo Logo', async ({ context }) => {
    const page = await context.newPage();
    // Start waiting for new page before clicking
    const pagePromise = context.waitForEvent('page');
    // Go to the starting url
    await page.goto(breakawayDemoAdLink);
    await expect(page).toHaveURL(breakawayDemoAdLink);
  ...
  test('Check click on Ad', async ({ context }) => {
    ...
    const newPage = await pagePromise;
    // await newPage.waitForLoadState();
    await expect(newPage).toHaveURL(breakawayAdLink, {timeout: 10000}); // URL contains
  });
