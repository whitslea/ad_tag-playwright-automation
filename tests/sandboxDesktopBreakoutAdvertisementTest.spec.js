...
const { kargoLink, desktopBreakoutAdvertisementAdLink, desktopBreakoutAdvertisementDemoAdLink } = require('../Ads Info/Ads Links');

test.describe('Sandbox Desktop Breakout Advertisement Ad Page', () => {
  test('Check and Click on Kargo Logo', async ({ context }) => {
    const page = await context.newPage();
    // Start waiting for new page before clicking
    const pagePromise = context.waitForEvent('page');
    // Go to the starting url
    await page.goto(desktopBreakoutAdvertisementDemoAdLink);
    await expect(page).toHaveURL(desktopBreakoutAdvertisementDemoAdLink);

    // Verifying Kargo logo section
    await expect(page.locator('a.kargo-hover-link.unfilled .kargo-svg-advertisement')).toBeVisible(); // check that kargo bolt logo is visible
  ...
  test('Check click on Ad', async ({ context }) => {
    ...
    const newPage = await pagePromise;
    // await newPage.waitForLoadState();
    await expect(newPage).toHaveURL(desktopBreakoutAdvertisementAdLink, {timeout: 10000}); // URL contains
  });
