...
const { kargoLink, desktopRunwayAdLink, desktopRunwayDemoAdLink } = require('../Ads Info/Ads Links');

test.describe('Sandbox Desktop Runway Ad Page', () => {
  test('Check and Click on Kargo Logo', async ({ context }) => {
    const page = await context.newPage();
    // Start waiting for new page before clicking
    const pagePromise = context.waitForEvent('page');
    // Go to the starting url
    await page.goto(desktopRunwayDemoAdLink);
    await expect(page).toHaveURL(desktopRunwayDemoAdLink);
  ...
  test('Check click on Ad', async ({ context }) => {
    ...
    const newPage = await pagePromise;
    // await newPage.waitForLoadState();
    await expect(newPage).toHaveURL(desktopRunwayAdLink, {timeout: 15000}); // URL contains
  });
