...
const { kargoLink, wideSkyscraperAdLink, wideSkyscraperDemoAdLink } = require('../Ads Info/Ads Links');

test.describe('Sandbox Wide Skyscraper Ad Page', () => {
  test('Check and Click on Kargo Logo', async ({ context }) => {
    const page = await context.newPage();
    // Start waiting for new page before clicking
    const pagePromise = context.waitForEvent('page');
    // Go to the starting url
    await page.goto(wideSkyscraperDemoAdLink);
    await expect(page).toHaveURL(wideSkyscraperDemoAdLink);
  ...
  test('Check click on Ad', async ({ context }) => {
    ...
    const newPage = await pagePromise;
    // await newPage.waitForLoadState();
    await expect(newPage).toHaveURL(wideSkyscraperAdLink, {timeout: 10000}); // URL contains
  });
  ...
  test('Check Ad Trackers', async ({ context }) => {
    ...
    // Verify that tthe first 6 trackers response status is 200
    for (let i = 0; i < 6; i++) {
