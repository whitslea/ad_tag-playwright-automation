...
const { kargoLink, middleBannerAdLink, middleBannerDemoAdLink } = require('../Ads Info/Ads Links');

test.describe('Sandbox Middle Banner Ad Page', () => {
  test('Check and Click on Kargo Logo', async ({ context }) => {
    const page = await context.newPage();
    // Start waiting for new page before clicking
    const pagePromise = context.waitForEvent('page');
    // Go to the starting url
    await page.goto(middleBannerDemoAdLink);
    await expect(page).toHaveURL(middleBannerDemoAdLink);
  ...
  test('Check click on Ad', async ({ context }) => {
    ...
    const newPage = await pagePromise;
    // await newPage.waitForLoadState();
    await expect(newPage).toHaveURL(middleBannerAdLink, {timeout: 10000}); // URL contains
  });
