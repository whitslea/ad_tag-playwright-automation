...
const { kargoLink, desktopHoverAdLink, desktopHoverDemoAdLink } = require('../Ads Info/Ads Links');

test.describe('Sandbox Desktop Hover Ad Page', () => {
  test('Check and Click on Kargo Logo', async ({ context }) => {
    const page = await context.newPage();
    // Start waiting for new page before clicking
    const pagePromise = context.waitForEvent('page');
    // Go to the starting url
    await page.goto(desktopHoverDemoAdLink);
    await expect(page).toHaveURL(desktopHoverDemoAdLink);
  ...
  test('Check click on Ad', async ({ context }) => {
    ...
    const newPage = await pagePromise;
    // await newPage.waitForLoadState();
    await expect(newPage).toHaveURL(desktopHoverAdLink, {timeout: 10000}); // URL contains
  });
