...
const { kargoLink, outstreamKAPAdLink, outstreamKAPDemoAdLink } = require('../Ads Info/Ads Links');

test.describe('Sandbox Outstream Video KAP Ad Page', () => {
  test('Check and Click on Kargo Logo', async ({ context }) => {
    const page = await context.newPage();
    // Start waiting for new page before clicking
    const pagePromise = context.waitForEvent('page');
    // Go to the starting url
    await page.goto(outstreamKAPDemoAdLink);
    await expect(page).toHaveURL(outstreamKAPDemoAdLink);
