...
const { kargoLink, socialCanvasAdLink, socialCanvasDemoAdLink } = require('../Ads Info/Ads Links');

test.describe('Sandbox Social Canvas Ad Page', () => {
  test('Check and Click on Kargo Logo', async ({ context }) => {
    const page = await context.newPage();
    // Start waiting for new page before clicking
    const pagePromise = context.waitForEvent('page');
    // Go to the starting url
    await page.goto(socialCanvasDemoAdLink);
    await expect(page).toHaveURL(socialCanvasDemoAdLink);
  ...
  test('Check click on Ad', async ({ context }) => {
    ...
    const newPage = await pagePromise;
    // await newPage.waitForLoadState();
    await expect(newPage).toHaveURL(socialCanvasAdLink, {timeout: 10000}); // URL contains
  });
